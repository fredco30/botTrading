...
void CheckPyramidClose() {
   ...
   for(int i = OrdersHistoryTotal() - 1; i >= 0; i--) {
      ...
      // --- NORMAL TRADE: update pyramid streak ---
      if(pnl > 0) {
         ...
         // --- TRIGGER REVERSE TRADE on L0 SL only ---
         if(UseReverseTrade && wasLevel == 0) {
...
//+------------------------------------------------------------------+
//| EXECUTE REVERSE TRADE — after L0 SL, open opposite direction     |
//| No signal, no filters. Market entry. SL = swing classic.         |
//| Lot = L0. Does not participate in pyramid streak.                |
//+------------------------------------------------------------------+
void ExecuteReverseTrade(int originalType) {
   // Check daily limit
   ...
   if(originalType == OP_BUY) {
      ...
      ExecuteTradeReverse(OP_SELL, price, sl, tp, "EMA Reverse Sell");
   }
      ...
      ExecuteTradeReverse(OP_BUY, price, sl, tp, "EMA Reverse Buy");
   }
}
...
//+------------------------------------------------------------------+
void ExecuteTradeReverse(int type, double price, double sl, double tp, string comment) {
   double slDist = MathAbs(price - sl);
   if(slDist <= 0) return;

   // Lot = configurable reverse multiplier (input RevLotMult)
   double riskMult = RevLotMult;
