//+------------------------------------------------------------------+
//|                                    PureReverse75_v2.mq4          |
//|           STRATÉGIE PURE: 1 Reverse = Récupération 75%          |
//|                                                                  |
//|  v2 CHANGE: CheckSLHit() now runs on every tick (not only       |
//|             on new bar). Instant REV trigger when SL hits,      |
//|             no more 1-bar delay on H1.                          |
//+------------------------------------------------------------------+
#property copyright "Pure Reverse System"
#property version   "2.00"
#property strict
#property description "Pure Reverse 75% v2 - Instant REV on SL hit"
...
input group "=== SYSTEM ==="
input int    Inp_Magic           = 112234;
input int    Inp_Slip            = 3;
input string Inp_Prefix          = "PR75v2";
...
void OpenReverse(double initialLossAmount) {
    ...
    // ★ CALCUL TP: Récupérer 75% de la perte initiale
    double targetProfit=initialLossAmount * Inp_Rev_RecoveryPct / 100.0; // 75% de la perte
    
    // Convertir en pips
    double tickVal=MarketInfo(Symbol(),MODE_TICKVALUE);
    double pipsNeeded=targetProfit/(tickVal*newLots);
    
    double tp;
    if(newType==OP_BUY)
        tp=Norm(entry+pipsNeeded*g_pt);
    else
        tp=Norm(entry-pipsNeeded*g_pt);
