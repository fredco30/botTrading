...
FAMILIES = [
    ("F06", "directional deceleration (pips-fixed)", F.extras_common, F.f06_feat,
     [("decel", F.f06_decel)]),
    ("F10", "yesterday-tight expansion breakout k=0.75", F.f10_build_extras, F.f10_feat,
     [("expand", F.f10_expand)]),
    ("F10", "yesterday-tight expansion breakout k=0.85", F.f10_build_extras, F.f10_feat,
     [("expand", F.f10_expand)]),
    ("F10", "daily breakout no tightness k=1.01", F.f10_build_extras, F.f10_feat,
     [("expand", F.f10_expand)]),
    ("F13", "NY-window breakout w/ day-trend align", F.f13_build_extras, F.f13_feat,
     [("nybrk", F.f13_nybrk)]),
    ("F01X", "serial-dep x vol regime", F.extras_common, F.f01_feat,
...
for fam_id, mech, bex, feat, configs in FAMILIES:
    if fam_id == "F10":
        F.F10_TIGHT_K = float(mech.split("k=")[1])
    try:
