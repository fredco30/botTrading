...
OUT_DIR = Path(r"E:\ResearchData\botTrading\nq\databento\highres\bbo_1s")
MANIFEST = HERE / "BBO_DOWNLOAD_MANIFEST.json"
DATASET = "GLBX.MDP3"
SCHEMA = "bbo-1s"
...
def main() -> int:
    ...
    man = json.loads(MANIFEST.read_text()) if MANIFEST.exists() else {
        "authorization": "BBO-1S ONLY, ceiling USD 4.00, human-approved 2026-09-16",
        "windows": {},
    }
    ...
    merged = wins.groupby(["DATE", "CONTRACT"], as_index=False).agg(
        START=("START", "min"), END=("END", "max")).sort_values("DATE")
    done = 0
